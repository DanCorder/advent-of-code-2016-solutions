namespace AdventOfCode
{
    ...
    public class Day09
    {
        private static readonly Regex MarkerRegex = new Regex(@"\((\d+)x(\d+)\)");

        public static int SolveProblem1()
        {
            // var toDecompress = @"(3x3)XYZ";
            var toDecompress = ProblemInput;
            var output = new StringBuilder();
            var index = 0;

            while (index < toDecompress.Length)
            {
                if (toDecompress[index] != '(')
                {
                    output.Append(toDecompress[index]);
                    index++;
                }
                else
                {
                    var markerMatch = MarkerRegex.Match(toDecompress, index);
                    var marker = markerMatch.Groups[0].Value;
                    var length = int.Parse(markerMatch.Groups[1].Value);
                    var repeptitions = int.Parse(markerMatch.Groups[2].Value);
                    var repetitionStart = index + marker.Length;
                    var repeatedText = toDecompress.Substring(repetitionStart, length);

                    for (var i = 0; i < repeptitions; i++)
                    {
                        output.Append(repeatedText);
                    }
        ...
        {
            PrintLength("(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN", 445);
            PrintLength("abc", 3);
            PrintLength("abc(10x3)(4x5)abcde", 66);

            return CalculateDecompressedLength(ProblemInput); // 10943094568
        }

        private static void PrintLength(string toPrint, int expectedLength)
        {
            System.Console.WriteLine("String '" + toPrint + "' has length " + CalculateDecompressedLength(toPrint) + " should be " + expectedLength);
        }

        private static long CalculateDecompressedLength(string toDecompress)
        {
            long index = 0;
            long length = 0;
            ...
            while (index < toDecompress.Length)
            {
                var compressedSubStringIndex = toDecompress.IndexOf('(', (int)index);

                if (compressedSubStringIndex == -1)
                {
                    length += (toDecompress.Length - index);
                    index += length;
                }
                else
                {
                    length += compressedSubStringIndex - index;
                    index = compressedSubStringIndex;

                    var markerMatch = MarkerRegex.Match(toDecompress, compressedSubStringIndex);
                    var marker = markerMatch.Groups[0].Value;
                    var repetitionlength = int.Parse(markerMatch.Groups[1].Value);
                    var repeptitions = int.Parse(markerMatch.Groups[2].Value);
                    var repetitionStart = compressedSubStringIndex + marker.Length;
                    var repeatedText = toDecompress.Substring(repetitionStart, repetitionlength);

                    length += (repeptitions * CalculateDecompressedLength(repeatedText));
                    index += (marker.Length + repetitionlength);
                }
            }
