namespace AdventOfCode
{
    ...
    public class Day04
    {
        ...
        {
            return ProblemInput.Select(r => new RoomDetails(r))
                                .Where(rd => rd.IsValid)
                                .Sum(rd => rd.SectorId);
        }
        ...
        private class RoomDetails
        {
            ...
            public RoomDetails(string detailsString)
            {
                var matchDetails = parser.Match(detailsString);
                encryptedName = matchDetails.Groups[1].Value;
                SectorId = int.Parse(matchDetails.Groups[2].Value);
                checksum = matchDetails.Groups[3].Value;
            }
            ...
            {
                var strippedChecksum = encryptedName.Replace("-", "");
                var checksumChars = strippedChecksum.GroupBy(c => c)
